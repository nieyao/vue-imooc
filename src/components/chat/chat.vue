    ...
    <div class="chat-main">
            ...
            <el-tabs type="border-card" >
                <el-tab-pane>
                    ...
                    <happyScroll resize size="8">
                        <div></div>
                        <friendCard  v-for="item in friendArray" @click.native="sendMsg(item.uid)" :headImg="item.headImg"  :id="item.uid" :account="item.account" :key="item.index" :name="item.name" :intro="'Hi，我是babyQ，你的智能小伙伴~ （注意认证标识，可别认错人哦\(^o^)/）'" ></friendCard>
                    </happyScroll>
                </el-tab-pane>
                    ...
                    <el-card v-for="(item,index) in msgArray" :key="index" shadow="always">
                        ...
                        <div v-if="item.type == -1">
                            <div slot="header" @click="sendMsg(item.friendId)" class="clearfix" style="height: 50px; display: flex; align-items: center; ">
                                <img style="height: 100%; display: block;" :src="linkPrefix + '/head/' + item.friendHead" />
                                <div style="margin-left: 20px; overflow: hidden;white-space: nowrap;text-overflow: ellipsis;" v-html="item.friendName + ' : ' + item.msgs[item.msgs.length - 1]"></div>
                            </div>
                        </div>
                    </el-card>
                ...
                <el-tab-pane label="查找好友">
                    ...
                    <happyScroll resize size="8">
                        ...
                        <friendCard v-for="item in searchArray" :type="true" :id="item.uid" :account="item.account" :key="item.index" :name="item.name" :headImg="item.headImg" :intro="'Hi，我是babyQ，你的智能小伙伴~ （注意认证标识，可别认错人哦\(^o^)/）'" ></friendCard>
                    </happyScroll>
                </el-tab-pane>
            </el-tabs>
        ...
        <div class="chat-console">
            <div class="chat-console-title" style="text-align: center; line-height: 50px; font-size: 20px; display: flex; justify-content: space-between; align-items: center;">
                ...
                <el-button v-if="showChat != null" style="width: 80px;height: 30px; margin-right: 20px;" @click="delFriend" size="mini" type="danger">删除好友</el-button>
            </div>
            <div id="chatContainer" class="chat-console-content">
                <div v-if="showChat != null">
                    <div v-for="(item,index) in chatArray" :key="index" style="width: 100%; height: 60px; line-height: 40px;" >
                        <div v-if="item.type === 1" style="magin: 10px 0;">
                            <div style="float: right; width: auto;">
                                <img style="width: 40px; margin: 0 20px" :src="linkPrefix + '/head/' + item.head" alt="">
                            </div>
                            <div style="float: right; height: 40px; line-height: 40px; display: flex; align-items: center; width: auto; padding: 0 10px; background: rgb(0,122,204); color: white; border-radius: 10px;" v-html="item.msg"></div>
                        </div>

                        <div v-else style="magin: 10px 0;">
                            <div style="float: left; width: auto;">
                                <img style="width: 40px; margin: 0 20px" :src="linkPrefix + '/head/' + item.head" alt="">
                            </div>
                            <div style="float: left; height: 40px; line-height: 40px; display: flex; align-items: center; width: auto; padding: 0 10px; background: rgb(0,122,204); color: white; border-radius: 10px;" v-html="item.msg"></div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="chat-console-input">
                <textarea ref="text" v-model="inputMsg" @keyup.enter="send()"  class="chat-console-input-textarea" style="min-width: 100%; max-width: 100%; min-height: 160px; max-height: 160px;resize: none;" ></textarea>
            </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            test(item) {
                console.log(faceMap)
                let key = Object.keys(faceMap)[item - 1];
                this.inputMsg += key;
            ...
            send() {
                ...
                this.chatArray.push({
                    msg: this.inputMsg.replace(/(<[\u4e00-\u9fa5]*>)/g,this.replacer),
                    head: user.headImg,
                    type: 1
                });
            ...
            sendMsg(id) {
                ...
                this.axios.get(this.linkPrefix + "/user/detail/" + id).then(res => {
                    ...
                    if(this.showChat != null && this.showChat.uid != res.data.content.uid) {
                        console.log("清空对话框")
                        this.chatArray = []
                    }
                    this.showChat = res.data.content;
                    this.loadMsg();
                    this.axios.get(`${this.linkPrefix}/message/read/${id}`).then(res => {
                    ...
                    for(let index in queues.friendMsgs) {
                        if(queues.friendMsgs[index].friendId === this.showChat.uid) {
                            ...
                            for(let msg in queues.friendMsgs[index].msgs) {
                                console.log('去消息', msg)
                                this.chatArray.push({
                                    msg: queues.friendMsgs[index].msgs[msg].replace(/(<[\u4e00-\u9fa5]*>)/g,this.replacer),
                                    head: queues.friendMsgs[index].friendHead,
                                    type: 0
                                });
                            }
